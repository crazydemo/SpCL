...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, num_slots=None):
        ...
        for _ in range(1):#self.iters
            ...
            dots_qk = dots_qk.softmax(dim=1) + self.eps
            dots_qk = dots_qk / dots_qk.sum(dim=-1, keepdim=True)

            dots_kv1 = torch.einsum('bid,bjd->bij', k, v1) * self.scale  # b, h*w, h*w
            dots_kv1 = dots_kv1.softmax(dim=1) + self.eps
            dots_kv1 = dots_kv1 / dots_kv1.sum(dim=-1, keepdim=True)

            dots_qk = dots_qk.unsqueeze(2).repeat([1,1,16*8,1])
            # print(dots_qk.size())
            # print("dots_qk", dots_qk.size())
            attn = dots_qk + dots_kv1.unsqueeze(1)

            # attn = attn.softmax(dim=1) + self.eps
            attn = attn / attn.sum(dim=-1, keepdim=True)
            # print("attn", attn.size())
            updates = torch.einsum('bkij,bid->bkjd', attn, v2) # b, k, hw, d
